(function() {
 ...
 ERNO.extend(ERNO.Slice.prototype, {get origin() {
    ...
    getColors: function (slice) {
            if (!slice)
                    return [];

            var d = this.indices.length, result = [];
            slice instanceof ERNO.Direction && (slice = slice.name);
            for(var i = 0; i < d; i++) {
                    var cubelet = this.getCubelet(i);
                    result.push((cubelet[slice].color.name).charAt(0));
            }
            return result;
    },
 });
